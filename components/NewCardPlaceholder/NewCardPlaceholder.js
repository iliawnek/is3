...
import {createTextCard, createChecklistCard, createImageCard, createVideoCard} from '../../core/reducers/projects';
...
export default class NewCardPlaceholder extends Component {
  ...
  createCard = (projectId, type, file) => {
    ...
    if (type === 'text') {
      ...
      createVideoCard(projectId, file);
    }
  };
  ...
  handleVideoUpload = (event) => {
    const file = event.target.files[0];
    if (file) {
      this.createCard(this.props.projectId, 'video', file);
    }
  };
